...
public class CodeGenerator {
	...
	public static void main(String[] args) {
		System.out.println("HappyCoding正在为您生成DAO实现类文件...");
		for (String temp : args) {
			if (temp.startsWith("--templatePath=")) {
				...
				++count;
				try {
					//Helper.getInstance().generateDaoImpl(temp, templatePath, templateName, outputPath);
					Helper.getInstance().generateDaoImpl(temp, templatePath, outputPath, templateName);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		}
		System.out.println("恭喜您！HappyCoding已成功生成"+count+"个DAO实现类文件...");
	}
}
