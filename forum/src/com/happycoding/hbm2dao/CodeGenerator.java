...
public class CodeGenerator {
	...
	public static void main(String[] args) {
		System.out.println("HappyCoding正在为您生成DAO文件...");
		for (String temp : args) {
			if (temp.startsWith("--templatePath=")) {
				...
				++count;
				try {
					Helper.getInstance().generateDao(temp, templatePath, templateName, outputPath);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
		}
		System.out.println("恭喜您！HappyCoding已成功生成"+count+"个DAO文件...");
	}
}
